...
function no200return(consulUrl) {
...
function importConsul(cas) {
    ...
    var files = document.getElementById('jsonInputFile').files,
        ...
        backendPath = localStorage['backendPath'],
        fr = new FileReader();
...
function updateFieldsToggle(readonly) {
    var cKeyValueObj = $('#cKeyValue'),
        ...
        keyValueFieldsid = $('#keyValueFieldsid'),
        userRights = $('#userRights').text();

    if (!readonly) {
        keyValueFieldsid.removeClass('border-warning');
        keyValueFieldsid.addClass('border-success');
        ...
        keyValueFieldsid.addClass('border-warning');
...
function dataValidation(data) {
    ...
    if (tobeFixedData.length > 0) {
        ...
        fixTree(tobeFixedData, backendPath, consulUrl);
        status = false;
    } else {
...
function getTree(tree, allKeys) {
    ...
    $.ajax({
        ...
        if (data['data'] === '[]') {
            ...
            if (dataValidation(realData, backendPath, consulUrl)) {
...
function exportConsul(data) {
    var consulUrl = localStorage['consulUrl'],
        backendPath = localStorage['backendPath'],
        selectedConsul = JSON.parse(localStorage['selectedConsul']),
...
function returnBackend(backendStatus) {
    var url;
    if (backendStatus === '') {
        url = "requests.php";
    } else {
        url = backendStatus + "requests.php";
    }
    return url;
}
        ...
        "remove": {
            ...
            "action": function (data) {
                ...
                if (confirm('Are you sure you want to DELETE ' + obj.id + ' ?')) {
                    ...
                    setTimeout(function () {
                        deleteNode(obj['id'], consulUrl, backendPath);
                    }, 250);
                }
            }
        }
